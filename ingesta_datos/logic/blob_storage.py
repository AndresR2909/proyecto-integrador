...
class BlobStorageManager:
    def __init__(self) -> None:
        try:
            ...
            self.container_client = blob_service_client.get_container_client(containerName)
        except Exception as e:
            logging.error(f"Error connecting to blob storage: {e}")
    
    def list_files_from_container(self):
        try:                    
            blob_list = [{'filename': elemento.name, 'lastupdate':elemento.last_modified } for elemento in self.container_client.list_blobs()]       
            return blob_list
        except Exception as e:
